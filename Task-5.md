# Task-5: AI 채팅 세션 UI 마무리 (이어서 하기 + 일기 생성 로딩 + 에러 처리)
...
roadmap.md의 **Task-5: AI 채팅 세션 UI 마무리** 항목을 처리한다. Task-3에서 채팅 패널을 모달로 통합하는 것까지는 완료됐고, 본 Task는 **사용자 경험 미결 사항 3건**을 마무리한다.
...
### 2.1 현재 상태와 Task-5 후 차이

| 영역 | 현재 (Task-4 완료 시점) | Task-5 후 |
| --- | --- | --- |
...
- [ ] **1.1** `backend/app/schemas.py`에 `QnAHistoryItem` 추가, `QnAStartResponse.history` 필드 추가
...
Task-5.md 작성 완료. **8개 Open Questions의 기본 답안에 동의하는지** 또는 변경할지 알려주세요. 변경 사항이 있으면 본 문서를 먼저 수정 후 코드 작업을 시작합니다. **승인 전까지 코드는 절대 수정하지 않습니다.**
