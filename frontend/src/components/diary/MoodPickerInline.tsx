...
export function MoodPickerInline({ date, initial }: MoodPickerInlineProps) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-around', padding: '12px 0', opacity: pending ? 0.7 : 1, transition: 'opacity var(--dur-1)' }}>
      {MOODS.map(mood => (
        <button
          key={mood}
          aria-label={mood}
          ...
          style={{
            ...
            background: selected === mood ? 'var(--sage-wash)' : 'transparent',
            transform: selected === mood ? 'scale(1.2)' : 'scale(1)',
            transition: 'transform var(--dur-1) var(--ease-soft), background var(--dur-1)',
