...
export function PetCard() {
  ...
  useEffect(() => {
    client.get('/pet').then(res => setPet(res.data)).catch(() => {})
  }, [])
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        <span style={{ color: 'var(--sage-ink)', fontWeight: 600 }}>{label}</span>
        {pet && (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 12 }}>XP</span>
              <div style={{ flex: 1, height: 6, background: 'var(--sage-wash)', borderRadius: 3, overflow: 'hidden' }}>
                <div
                  style={{
                    height: '100%',
              ...
              <span style={{ fontSize: 12 }}>{pet.xp}/{pet.xp_to_next}</span>
            </div>
          </>
        )}
      </div>
