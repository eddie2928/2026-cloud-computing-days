"""agentbox set — unified environment provisioning."""
import logging
import os
import platform
import subprocess
...
def _setup_file_logger() -> None:
    ...
    handler = logging.FileHandler(log_dir / f"agentbox-set-{timestamp}.log")
...
def _check_deps_step(args) -> int:
    """Step 1: Check system dependencies. Returns 0 on success/skip, 4 on hard fail."""
    failed = []
    for dep in DEPS:
...
def _check_env_step(args, bashrc: Path) -> int:
    """Step 2: Check AWS env vars, add missing ones to ~/.bashrc."""
    ...
    if lines_to_add:
        ...
        _log("[agentbox] Step 2: 환경변수 모두 OK.")

    return 0


def _check_ca_step() -> int:
    """Step 3: Ensure CA cert exists; warn if not in system trust store."""
...
def _install_shell_integration(bashrc: Path) -> bool:
    """Step 4: Add agentbox on/off shell functions to ~/.bashrc.

    Returns True if newly added, False if already present.
    NOTE: Never adds HTTPS_PROXY auto-export or auto-activation.
    """
...
def run_set(args) -> int:
    ...
    _log("[agentbox] Step 1: 의존성 점검...")
    rc = _check_deps_step(args)
    if rc != 0:
        return rc

    _log("[agentbox] Step 2: 환경변수 점검...")
    _check_env_step(args, bashrc)

    _log("[agentbox] Step 3: CA 인증서 확인...")
    try:
        _check_ca_step()
    except Exception as exc:
        _log(f"[agentbox] CA 확인 중 오류 (무시 가능): {exc}", "warning")

    _log("[agentbox] Step 4: Shell integration 등록...")
