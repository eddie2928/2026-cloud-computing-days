...
- [x] Task 2: 취향 API (GET/PUT) + 스키마 — type: NEW_FEATURE, worker: jmh-worker-backend, depends: Task 1
- [x] Task 3: 추천 Stub 모듈 + /api/recommend/songs — type: NEW_FEATURE, worker: jmh-worker-backend, depends: Task 2
- [x] Task 4: 음원 프록시 /api/music/search (iTunes) + httpx — type: NEW_FEATURE, worker: jmh-worker-backend, depends: none
